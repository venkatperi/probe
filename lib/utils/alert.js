...
define( function ( require, exports, module ) {
  ...
  var Alert = function ( opts, extra ) {
      ...
      case 'smart':
        ...
        this.cmp = function ( value, threshold ) {
          if ( value !== null ) {
            ...
            //return false
            {
              return (Math.exp( -Math.pow( value - this.sample.getMean(), 2 ) / (2 * this.sample.getVariance()) )
                      / (Math.sqrt( 2 * Math.PI ) * this.sample.getStddev()) < 0.00000001)
            }
          }
          return false;
        };
    ...
    this.msg =
        opts.msg || ((extra && extra.name) ? ('Probe ' + extra.name + ' has reached value') : 'Alert value reached');
  ...
  Alert.prototype.tick = function ( value ) {
    ...
    if ( this.reached === 0 ) {
      ...
      if ( value === null && this.nullCheck < 10 ) {
        ...
        if ( this.func ) {
          setTimeout( function () {self.func( value )}, 50 );
        }
        this.reached = 1;
        this.nullCheck = 0;
      }
